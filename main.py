...
def main():
    ...
    cases = [\
        ...
        ##{'scheme': 'ImExRK', 'RK':'UJ31e32', 'SD':'fifth302', 'blend':'sm', 'iterFCT':False, 'nIter':1, 'ymin':0., 'ymax':10000., 'posdefFCT':True},#, 'output_substages':True},
        #{'scheme': 'ImExRK', 'RK':'UJ31e32', 'SD':'fifth302', 'blend':'sm', 'iterFCT':True, 'nIter':1},#, 'output_substages':True},
    ...
    plot_args = [\
        ...
        ##{'label':'AdImEx Strang FCT PD', 'color':'purple', 'marker':'x', 'linestyle':'-'},
        #{'label':'AdImEx Strang FCT', 'color':'purple', 'marker':'x', 'linestyle':'-'},
        #{'label':'AdImEx Strang 2FCT', 'color':'red', 'marker':'x', 'linestyle':'-'},
        #{'label':'AdImEx Strang 3FCT', 'color':'orange', 'marker':'x', 'linestyle':'-'},
        ]

    # Initial conditions
    ymin, ymax = -0.2, 1.5#200.#2. #8., 13.        # field min and max for plotting purposes (animation)
    nx = 40#400#128#40                     # number of points in space
    xmax = 1.#1000. #m #1.                   # physical domain parameters
    nt = 1 #00 #32#16#2#32#100#10000#500#100 #50#100                      # number of time steps # needs to be 1 when output_substages is True for ImExRK scheme
    dt = 0.01#0.001#2.#0.2 #s #0.2#0.01                   # time step
    ...
    analytic = an.combi #cosine_bell_x#an.combi#sine_xyshiftampl4#an.combi#double_square   # initial condition, options: sine, cosbell, tophat, or combi, halfwave, revhalfwave, and more for varying velocity field
    u_setting = 'constant'#'varying_space7'#'constant'#'varying_time_space_swift_2Dnondiv'#_sin6'# 'constant' or various 'varying_space..' options
    ...
    if u_setting == 'constant':
        uconstant = 6.25# 3.125 #1.#3.125#6.25#3.125#250.#50.#1.        # constant velocity # should only apply when u_setting == 'constant' # is used in the analytic function and for the title in the final.pdf plot for the constant velocity field
